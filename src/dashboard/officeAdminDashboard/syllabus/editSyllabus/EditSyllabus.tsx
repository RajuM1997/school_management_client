...
const EditSyllabus = () => {
  ...
  const { data } = useFetch<BookListProps | null>(
    `${import.meta.env.VITE_REACT_APP_BASE_URL}/syllabus/${id}`
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        setTimeout(() => {
          navigate("/office-admin/syllabus");
        }, 2000);
      }
    } catch (error) {
  ...
  return (
    <div className="dashboard_containerBefore">
      <Container maxWidth="xl">
        ...
        >
          <h2 className="common_title">Edit Syllabus</h2>
          <Box className="dashboard_container_input">
            <div className="type_form">
              ...
              <Box className="type_submit_btn">
                <button className="common_dashboard_btn" type="submit">
                  Update
                </button>
              </Box>
            </div>
