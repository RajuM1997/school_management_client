...
const EditExamRouting = () => {
    ...
    try {
      axios
        .delete(
          ...
          }/delete/${file}?exam-routing=exam-routing`
        )
        .then(() => {});
    } catch (error) {}
  };
  const handleSubmit = async () => {
    ...
    if (files) {
      await Promise.all(
        Object.values(files).map(async (file) => {
          ...
          imgData.append("exam-routing", file);
          try {
    ...
    try {
      ...
      if (res.status === 200) {
        addSuccessfully("Successfully update");
        setTimeout(() => {
  ...
  return (
    <div className="dashboard_containerBefore">
      <Container maxWidth={"xl"}>
        ...
        >
          <h2 className="common_title">Edit Exam Routing</h2>
...
export default EditExamRouting;
